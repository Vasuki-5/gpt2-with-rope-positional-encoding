    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is not None:
            ...
            query_states = query_states.view(shape_q).transpose(1, 2)  # [B, H, T, D]
            key_states = key_states.view(shape_kv).transpose(1, 2)  # [B, H, T, D]

            from rope import apply_rope
            seq_len = query_states.size(2)
            query_states = apply_rope(query_states, seq_len)
            key_states = apply_rope(key_states, seq_len)
            # ====== ✅ Done with RoPE ======
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = inputs_embeds  # Use token embeddings only
